...
# Maximum length for each summary message part to avoid exceeding Telegram's message size limit (4096 chars)
MAX_MESSAGE_PART_LENGTH = 3800
                    ...
                    for i, part in enumerate(summary_parts):
                        ...
                        for attempt in range(MAX_SEND_RETRIES):
                            try:
                                ...
                                if attempt < MAX_SEND_RETRIES - 1:  # 不是最后一次尝试
                                    logger.warning(f"触发 Telegram 发送频率限制，等待 {fwe.seconds} 秒后重试...")
                                    await asyncio.sleep(fwe.seconds)
                                else:
                                    raise  # 最后一次尝试失败，向上抛出异常
                                    
                            except errors.MarkupInvalidError as e:
                                logger.warning(f"Markdown 解析失败: {e}")
                                # 降级为纯文本发送
                                current_message = await self.bot_client.send_message(
                                    target_chat_id,
                                    message_to_send
